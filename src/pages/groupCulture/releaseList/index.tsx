...
import { Image, Space, Tag, message, Popconfirm } from 'antd';
...
const ReleaseList = memo(() => {
  ...
  useEffect(() => {
    if (id) {
      ...
      const options = Object.assign({}, pages, {
        params: { associationId: id, msg: queryValue },
      });
      getReleaseList(options).then((res) => {
  ...
  async function onModalSubmit(values: {
    ...
    if (userJson && userJson.username) {
      ...
      const data = await updateReleaseList(options);
